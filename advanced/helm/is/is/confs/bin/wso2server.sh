...
cp -RL /home/wso2carbon/data/* ${CARBON_HOME}/
sleep 2
rm -rf /home/wso2carbon/data/*

file="${CARBON_HOME}/password-tmp"

tmpPass=$( cat $file )

if [ ! -z "$tmpPass" ]; then 
    sh ${CARBON_HOME}/bin/ciphertool.sh -Dconfigure -Dpassword=$tmpPass
fi

if [ "$?" != "0" ]; then
    rm -rf ${CARBON_HOME}/password-tmp
    rm -rf ${CARBON_HOME}/repository/conf/security/cipher*
    echo " Password encryption failed .. !!" 1>&2
    echo " "
    exit 1
else
echo "Password encryption finished .. !"
echo " "
fi
